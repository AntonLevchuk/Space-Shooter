...
import { bulletTick, destroyBullet, initBullets, destroyAllBullets } from './sprites/bullets'
import { addPlayer, getPlayer, playerShoots, playerTick, playerShots, resetPlayerShots} from './sprites/player'
import { initAsteroids, addAsteroid, asteroidsTick, destroyAsteroid, asteroidsLeft, resetAsteroids} from './sprites/asteroids'
...
import { initBoss, addBoss, bossTick, resetBoss, addHealthBar, bossTakeDamage, bossHp, destroyBoss } from './sprites/boss'
...
const createScene = () => {
    ...
    playerBullets = allBullets.getChildByName(appConstants.containers.playerBullets);
    gameSceneContainer.addChild(playerBullets);

    bossBullets = allBullets.getChildByName(appConstants.containers.bossBullets);
...
const checkAllCollisions = () => {
    const asteroids = gameSceneContainer.getChildByName(appConstants.containers.asteroids);
    const playerBulets = gameSceneContainer.getChildByName(appConstants.containers.playerBullets);
    const bossBullets = gameSceneContainer.getChildByName(appConstants.containers.bossBullets);
    const boss = gameSceneContainer.getChildByName(appConstants.containers.bossContainer);
    const player = gameSceneContainer.getChildByName(appConstants.containers.player);
...
const endRound = () => {
    if (startBossRound && playerHp <= 0) {
        changeScene(appConstants.text.looseRoundText);
        changeButtonText(appConstants.text.looseRoundButtonText);
    }
    if (timer.isFinish || (asteroidsLeft > 0 && (playerShots >= appConstants.amount.playerBoolets && playerBullets.children.length == 0))) {
        changeScene(appConstants.text.looseRoundText);
        changeButtonText(appConstants.text.looseRoundButtonText);
...
const iterateColisionElements = (_obj1, _obj2) => {
    if (_obj2.name == appConstants.containers.player) {
        _obj1.children.forEach((_j) => {
            if (_j.parent.name == appConstants.containers.bossBullets && checkCollision(_j, _obj2)) {
    ...
    _obj1.children.forEach((b) => {
        _obj2.children.forEach((a) => {
            if (checkCollision(b, a)) {
                if (a.parent.name == appConstants.containers.asteroids && b.parent.name == appConstants.containers.playerBullets) {
                    destroyAsteroid(a.name);
                    destroyBullet(b.name);
                } else if (b.parent.name == appConstants.containers.asteroids && a.parent.name == appConstants.containers.playerBullets) {
                    destroyAsteroid(b.name);
                    destroyBullet(a.name);
                } else if (b.name == appConstants.containers.boss && a.parent.name == appConstants.containers.playerBullets) {
                    bossTakeDamage();
                    destroyBullet(a.name);
                } else {
                    destroyBullet(b.name);
